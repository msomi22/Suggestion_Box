...
def getonesuggestion(suggestid):
    ...
    return render_template('suggestion.html', suggestion=suggestion, vote=vote , flag=flag)
...
def signup():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if not validate_email(form.email.data) and form.validate:
            return render_template('index.html',form=form, message='The email provided is not valid')    

        if existEmail(form.email.data) and form.validate:
            return render_template('index.html',form=form, message='The email already exist!')    
        
        if existUsername(form.username.data):
            return render_template('index.html',form=form, message='The username already exist!')
        ...
        return render_template('index.html', form=form , message='Registration successful, you can login')
       
    return render_template('index.html', form=form , message='Something went wrong try again')
...
def login():
    ...
    return render_template('index.html', form=form, message='Incorrect user credentials, try again')    
...
def suggestion():
    ...
    if request.method == 'POST' and form.validate():
        ...
        models.db.session.commit()
        return render_template('home.html', form=form , message='Suggestion posted successfully') 
    return render_template('suggest.html', form=form , messgae='Something went wrong')     
